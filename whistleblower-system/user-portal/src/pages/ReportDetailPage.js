...
import { getReportById, getVoiceNote } from '../api/whistleblower';
 ...
 useEffect(() => {
   ...
   const fetchReport = async () => {
     try {
       ...
       // If report has a voice note, fetch it
       if (data.hasVoiceNote && data.voiceNote) {
         try {
           const audioBlob = await getVoiceNote(data.voiceNote);
           const url = URL.createObjectURL(audioBlob);
           setVoiceNoteUrl(url);
           audioRef.current.src = url;
         } catch (audioError) {
           console.error('Error fetching voice note:', audioError);
         }
       }
   ...
   <div className="container">
     <div className="report-detail">
       ...
       <div className="report-info">
         ...
         {report.hasVoiceNote && voiceNoteUrl && (
